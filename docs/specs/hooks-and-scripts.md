...
| 파일 | 대상 | 내용 |
|---|---|---|
| **`hooks.json`** | Claude (플러그인) | **Claude Code가 플러그인 루트의 이 경로를 관례로 탐색해** 마켓플레이스 설치 시 자동 등록한다 — `.claude-plugin/plugin.json`에 `hooks` 키는 **두지 않는다**(선언 + 관례 파일이 둘 다 있으면 중복 등록된다. 2026-08-01 실측: `hooks` 키가 없고 `~/.claude/settings.json`에도 훅 이벤트가 없는 설치본에서 이 파일의 command가 그대로 발화했다). SessionStart·PreToolUse·PostToolUse 3종을 `${CLAUDE_PLUGIN_ROOT}/hooks/run-hook.cmd` 경유로 등록 → **install.sh 없이 동작**. `--fallback`(수동)은 `${CLAUDE_PLUGIN_ROOT}`를 `~/.claude`로 치환한 스니펫을 만들어 `~/.claude/settings.json` 머지를 안내. |

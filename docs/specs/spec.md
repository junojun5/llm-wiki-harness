...
**설치 (플랫폼별로 다르다):** **Claude·Codex는 플러그인 마켓플레이스**가 기본 경로다 — 훅·스킬이 플러그인 루트(`${CLAUDE_PLUGIN_ROOT}` / `${PLUGIN_ROOT}`) 경유로 자동 등록되고, 첫 SessionStart가 플러그인 루트의 `scripts/`를 도구 비종속 런타임 홈 `~/.llm-wiki/scripts/`로 부트스트랩하므로 **install.sh 없이 동작**한다. **Cursor는 플러그인으로 훅을 등록할 수 없다**(2026-07-31 실측, §5-0) — `.cursor-plugin/`은 스킬만 싣고, 훅은 `install.sh`가 `~/.cursor/hooks.json`을 배치해야 한다. 즉 **Cursor에 한해 install.sh는 폴백이 아니라 필수**다. **Antigravity**는 훅 스키마 미공개로 install.sh가 `~/.llm-wiki` 부트스트랩과 스킬·rules 배치를 담당한다. `install.sh` 기본 실행은 `~/.llm-wiki` 부트스트랩 + Antigravity 배치, `--fallback`은 Claude/Codex/Cursor 홈 전역 배치, `--vault`는 프로젝트 로컬 배치다. **기존 설정 파일은 절대 덮어쓰지 않는다** — 공유 JSON 등록 파일(`~/.claude/settings.json`·`~/.codex/hooks.json`·`~/.cursor/hooks.json`·볼트 로컬 2종·`.cursor/sandbox.json`)은 **마커 기반으로 멱등 병합**해 우리 항목만 교체하고 남의 항목·무관 키는 보존한다. 병합할 수 없는 것(마크다운, 파싱 불가한 JSON)만 `<원본명>.llm-wiki.<확장자>`로 옆에 두고 머지를 안내한다. 플랫폼 매핑·매니페스트 상세는 `docs/distribution-design.md` §4·§7이 담당한다. 어느 방식이든 canonical source는 git repo이고 런타임 타깃은 그 사본(symlink 또는 플러그인 루트 참조)일 뿐이므로:
...
**`render()`의 실패 모양이 특히 나쁘다 (누락이 남긴 교훈).** `open(dest,"w")`가 먼저 열리고 그 다음 `open(src).read()`가 죽으므로, dest는 **0바이트 파일로 남는다.** 훅 등록 파일이 "없음"이 아니라 "빈 파일"이 되어 설치는 성공한 것처럼 보이고 가드만 조용히 비활성화된다. 즉 §3-9 누락의 대가는 예외 하나가 아니라 **가드 무력화**다 — 새 python3 호출 지점을 추가할 때 이 계약을 먼저 확인한다. 회귀 고정: `tests/install/smoke.sh` [11]. 그리고 누락 자체를 막기 위해 **`tests/scripts/test-python-utf8-contract.sh`가 레포 전체를 스캔**한다 — 사람이 "적용 대상" 목록을 관리하면 또 빠진다(실제로 `install.sh`가 그렇게 빠졌다). 새 python3 호출 지점이 추가되는 순간 실패하므로 §3-9가 문서가 아니라 계약이 된다.
...
| 2026-08-04 | **공유 등록 파일은 사본이 아니라 마커 기반 멱등 병합으로 통합한다 (배포 설계 §7-1a).** `install.sh`의 `merge_json` 신설 — 대상 6곳(`~/.claude/settings.json` 포함) | "원본 보존 + `.llm-wiki` 사본 + 수동 머지 안내"는 덮어쓰지 않아 안전했지만 **통합을 사용자 손에 남겼다** — 다른 레포도 같은 정책을 쓰면 사본만 쌓이고 훅은 하나도 등록되지 않은 채 설치가 "성공"한다. §5-5 가드 생존 문제의 또 다른 입구다(설치 성공 ≠ 가드 작동). 마커를 **절대경로가 아니라 런처+훅 스크립트명**으로 잡은 것이 핵심 — 경로 마커는 설치 위치가 바뀔 때 자기 항목을 못 찾아 중복을 쌓는다(`conda init`의 conda#8703이 그 사례). 남의 항목 삭제가 최악의 실패이므로 마커는 두 조건을 **동시에** 요구하고, 파싱 불가한 대상은 손대지 않고 사본으로 강등한다. 드롭인(`.d`)이 있으면 병합보다 우선하나 세 플랫폼 모두 훅 설정 드롭인을 공개하지 않는다(실측 전 채택 보류). 대가로 §10에 항목 하나가 열린다 — Cursor가 `~/.claude/settings.json`도 실행하므로 `--fallback`에서 2회 발화가 가능하다(미실측) |

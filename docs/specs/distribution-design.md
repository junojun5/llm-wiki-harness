...
- **Windows `.ps1`/`.bat` 패리티 (향후)** — 1차는 Git Bash/WSL 요구로 처리. 네이티브 cmd/PowerShell 에이전트 수요가 확인되면 `resolve-vault`·`validate-frontmatter`의 PowerShell 대응본 + 런처 OS 분기 추가.
  ...
  - **`bash-suite`가 드러낸 두 번째 결함 — Windows Python의 기본 인코딩이 cp1252다 (2026-08-04, 미수정).** `UnicodeDecodeError: 'charmap' codec can't decode byte 0x9d` 가 여러 스위트에서 나왔다. 이건 테스트만의 문제가 아니다 — **프로덕션 코드에 두 표면이 있다:**

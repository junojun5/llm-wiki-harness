...
- **Windows:** 공유 스크립트는 `.sh`(bash) → **Git for Windows**가 필요함을 README에 명시(2026-08-05 정정: ~~"또는 WSL bash"~~ — WSL 갈래는 실기 검증된 적이 없고, System32의 레거시 `bash.exe`는 런처가 넘기는 `C:\…` 경로를 해석하지 못한다. `run-hook.cmd`가 Git 설치 경로를 먼저 뒤지고 `where bash`는 마지막 수단으로 남긴다). `.ps1`/`.bat` 패리티 버전 + 런처 OS 분기는 향후 보완 항목(§10).
- `--update-path`(전역 볼트 재지정), `--repair` 등 하네스 스펙 §3-1 플래그는 `wiki-setup` 스킬이 담당. install.sh는 *배포*만, 볼트 *설정*은 wiki-setup.
- 멱등(idempotent): 재실행 안전, 기존 배치 갱신.

#### 7-1b. 배치 원장 — 소유권을 파일시스템에 묻지 않는다 (2026-08-07 신설)

**계약:** 하네스가 놓은 자리는 `~/.llm-wiki/.placements`에 한 줄로 기록한다. 소유권 판정은 **이 원장만** 본다.

```
<mechanism>\t<dest>\t<src>          # mechanism = symlink | copy, dest·src는 절대경로(탭 없음)
```

- **왜:** 종전 판정은 `[ -L "$dest" ]`(“symlink인가”)였다. MSYS(Git Bash)의 `ln -s`는 진짜 symlink가 아니라 **복사본**을 만들므로(진짜 symlink는 `MSYS=winsymlinks:nativestrict` + 개발자 모드/관리자 권한 필요) Windows에서 판정이 **항상 “사용자 파일”로 떨어졌다.** 세 증상이 한 뿌리였다: ① 2회차 install이 자기 산물에 사이드카 3건을 만들고 **병합할 게 없는데 수동 머지를 요구**했다 ② 3회차가 `ln: …/skills/ingest-url/ingest-url: cannot overwrite directory`로 즉사해 **그 뒤 `[3]` 훅 등록 전체가 실행되지 않았다**(dest가 실존 디렉터리면 `ln`은 그 *안쪽*을 노리고 `-f`는 디렉터리를 못 지운다) ③ `session-start` 부트스트랩이 재지정을 못 해 **런타임 홈이 영구히 stale**해졌다(§5-0). 전부 fail-open이고 조용하다.
- **선행 사례:** Homebrew `INSTALL_RECEIPT.json`, dpkg `.list`, npm `node_modules/.package-lock.json`. 이 레포도 ingest 쪽은 이미 같은 패턴이다(`.manifest.json`). 참고로 `place_render`는 처음부터 `cmp`로 **내용**을 비교했고 Windows에서 통과했다 — 메타데이터로 묻는 함수만 실패했다.
- **배치는 결과를 검증한다:** `ln -sfn` 후 `[ -L ]`로 확인하고, 아니면 복사로 폴백해 `copy`로 기록한다. 플랫폼 분기가 아니라 결과 확인이므로 코드 경로가 하나다.
- **소유권 없는 자리는 건드리지 않는다.** `install.sh`의 무조건 배치 경로(`link()`, 15개 호출 지점)도 이제 소유권을 확인한다 — 확인 없이 배치하면 어느 쪽으로도 안전하지 않다: dest를 비우면 사용자 디렉터리를 지우고, 비우지 않으면 `ln`이 사용자 트리 **안쪽**을 오염시킨다(후자는 종전 동작의 잠재 결함이었다).
- **마이그레이션:** 원장이 없고 dest가 `src`를 가리키는 **진짜 symlink**면 원장 도입 이전 설치로 보고 흡수한다. 이 갈래가 없으면 기존 macOS/Linux 사용자 전부가 다음 실행에서 사이드카를 뒤집어쓴다.
- **`session-start`는 원장 I/O를 bash 내장만으로 구현한다** — `grep`·`cut`·`cp`·`mv`·`rm` 금지. 이 훅은 “python3가 없다”를 알리는 경로여서 풍부한 환경을 가정할 수 없고, `tests/hooks/test-session-start.sh`가 PATH를 `bash dirname head sed readlink ln mkdir cat env`로 좁혀 그 계약을 검증한다. 헬퍼를 `install.sh`와 **공유하지 않고 복제**하는 이유도 자족성이다: 이 훅은 마켓플레이스 캐시에서 단독 실행되고 사본으로 배포되면 형제 파일을 찾을 수 없다.
- **테스트는 메커니즘을 단언하지 않는다.** `[ -L ]` 단언 8건을 `tests/lib/placement.sh`의 `placed()`(존재 + 원장 기록)로 교체했다 — 그 단언들이 Windows 실패의 절반(smoke 5건·session-start 2건)이었고, 진짜 결함이 그 소음에 섞여 있었다.
- **회귀:** `tests/install/test-msys-placement.sh`가 MSYS `ln -s` 시맨틱을 PATH 셰임으로 재현해 15단언으로 고정한다(3회 연속 install · 사이드카 0 · 사용자 파일 보존 · 사용자 디렉터리 비파괴 · 이전 설치 흡수 · 부트스트랩 재지정). windows-latest 러너에는 Git Bash가 항상 있어 이 상태를 CI에서 만들 수 없으므로 셰임이 유일한 재현 수단이고, 결함이 플랫폼이 아니라 **판정 로직**에 있으므로 macOS/Linux에서도 같은 경로를 밟는다.
...
- **Windows `.ps1`/`.bat` 패리티 (향후)** — 1차는 **Git for Windows** 요구로 처리(~~Git Bash/WSL~~ — 2026-08-05 정정, 아래 항목 참조). 네이티브 cmd/PowerShell 에이전트 수요가 확인되면 `resolve-vault`·`validate-frontmatter`의 PowerShell 대응본 + 런처 OS 분기 추가.
  - **cmd.exe 분기는 Claude에서 "예외 경로"다 — 공식 문서 확인 (2026-08-05).** Claude Code 훅 항목에는 `shell` 필드가 있고 **기본값이 `bash`**이며, Windows에서 **Git Bash가 없을 때만** `powershell`로 떨어진다(`code.claude.com/docs/en/hooks`). 즉 Git Bash가 있으면 Claude는 `run-hook.cmd`를 **bash로 실행**해 폴리글랏의 Unix 분기를 탄다 — `:344`의 CRLF + `' #'` 페어가 Windows Claude 경로에서도 필수인 이유이고, 동시에 **required로 둔 `cmd-launcher` 잡이 덮는 것은 덜 흔한 경로**라는 뜻이다. cmd.exe 분기가 실제로 쓰이는 쪽은 ① Git Bash 없는 Claude(→PowerShell→`.cmd`) ② Codex(`$SHELL -lc`)·Cursor다. **`shell`을 명시하지 않는 것도 계약이다:** `"bash"`로 못박으면 Git Bash 없는 Windows에서 PowerShell 폴백이 사라져 매 호출이 훅 에러가 된다. 이 사실은 obra/superpowers 대조에서 나왔다 — 그쪽은 `shell: "bash"`를 명시해 cmd.exe 경로를 봉인했고, 그래서 같은 폴리글랏을 **LF로** 배포하면서도 깨지지 않는다(그들의 배치 블록은 사실상 사문이며 LF라 실제로 돌면 `:343`과 같은 이유로 깨질 것이다). 우리는 Codex·Cursor·PowerShell 폴백 때문에 그 봉인을 쓸 수 없으므로 CRLF를 유지한다.
  - **bash 탐색은 Git 설치 경로 우선, `where bash`는 마지막 수단 (2026-08-05 수정).** 종전 런처는 `where bash`만 썼다. WSL 기능이 켜진 시스템에는 `C:\Windows\System32\bash.exe`(레거시 런처)가 있고 System32는 PATH 앞쪽에 거의 항상 있으므로, 그걸 집으면 런처가 넘기는 `%~dp0` 형식의 `C:\…` 경로를 WSL 안에서 해석할 수 없어 훅이 **조용히** 죽는다. 탐색 순서를 `%ProgramFiles%\Git\bin\bash.exe` → `%ProgramFiles(x86)%\…` → `%LOCALAPPDATA%\Programs\Git\bin\bash.exe` → `where bash`로 바꿨다(obra/superpowers의 하드코딩 선탐색에서 채택). 함께 **README·troubleshooting·`:248`의 "Git Bash 또는 WSL bash" 문구를 정정**했다 — WSL 갈래는 실기 검증된 적이 없는 주장이었다. ⚠️ **미검증:** WSL bash가 실제로 실패한다는 것은 추론이고 실측이 아니다(windows-latest 러너에는 Git Bash가 항상 있어 그 상태를 만들 수 없다). 닫는 방향은 PATH를 스크럽한 WSL-only 잡을 추가하는 것.
  - **bash 부재 시의 exit 코드는 훅 역할로 갈린다 (2026-08-05 수정).** 종전 런처는 무조건 `exit /b 1`이었다. 비영 exit는 non-blocking error이고 PreToolUse matcher가 `Write|Edit|MultiEdit|NotebookEdit|Bash`로 **전역**이므로, Git Bash 없는 Windows에서는 **볼트와 무관한 아무 편집에서도 매 호출마다 "hook error" 알림**이 떴다. 반대로 `exit /b 2`로 fail-closed 하면 그 전역 matcher가 모든 편집을 차단해 도구를 쓸 수 없게 만든다 — 둘 다 못 쓴다. 채택안: `session-start`는 stderr + `exit 2`(SessionStart는 exit 2로 차단되지 않고 stderr만 노출 → **세션당 1회** "가드 비활성" 경고), 가드 2개는 **무음 `exit 0`**("판정할 수 없으면 조용히 통과"라는 `wiki-protect-raw.sh`의 resolver 실패 정책과 일관). superpowers는 무조건 무음 `exit 0`인데 그건 가드 훅이 없어서 성립하는 선택이다. 경고 문구는 **ASCII**로 쓴다 — 배치 파일의 한글 리터럴은 활성 코드페이지에 따라 mojibake가 되고(`:320`의 비ASCII WARN과 같은 이유), 이 문장은 사용자가 조치해야 하는 유일한 출력이다. ⚠️ **미검증:** 위와 같은 이유로 bash 부재 상태를 CI에서 만들 수 없어 정적 검토만 됐다.
  - **런처 경로 인용 통일 (2026-08-05 수정).** `hooks-codex.json`만 command의 런처 경로를 큰따옴표로 감쌌고 `hooks.json`·`hooks-cursor.json`은 아니었다 — 세 파일이 서로 달랐다는 것 자체가 결함 신호다. command는 셸 문자열로 실행되므로 플러그인 루트 경로에 공백이 있으면(Windows 사용자명·macOS 홈) 인자 경계가 깨진다. 세 파일을 인용으로 맞추고 `tests/hooks/test-hook-config-schema.sh`에 계약으로 고정했다. Claude(`shell` 기본 bash)·Codex(`$SHELL -lc`)는 문서로 확정이고, **Cursor는 문서가 command를 "shell string"으로 기술할 뿐 실행 모델을 명시하지 않아 미실측이다** — 셸을 거치면 인용이 맞고 argv 단순 분할이면 인용이 오히려 깨뜨린다(다만 그 경우 공백 경로는 인용 없이도 이미 깨진다). cursor-agent로 sessionStart를 발화시켜 확인해야 하며, 어긋나면 `hooks-cursor.json`의 큰따옴표 3쌍만 되돌린다.
  ...
  - ~~**MSYS `ln -s` 시맨틱 — 소유권 판정이 Windows에서 무너진다**~~ → **해결 (2026-08-07, §7-1b 배치 원장).** `[ -L ]` 판정을 `~/.llm-wiki/.placements` 원장으로 교체했다. 닫힌 증상 3건: ① 2회차 install의 사이드카 3건 오판(`✗ AGENTS 사본 없음`의 뿌리 — `:346`이 "별개 항목"으로 미뤄둔 것) ② 3회차 `ln: cannot overwrite directory` 즉사로 훅 등록 구간 미실행 ③ `session-start` 부트스트랩 재지정 불가로 런타임 홈 영구 stale. 함께 잡은 **잠재 결함:** `link()`이 소유권을 확인하지 않아 사용자 디렉터리 안쪽에 중첩 배치했다(macOS/Linux에서도 조용히 일어났다). 채택하지 않은 대안: `MSYS=winsymlinks:nativestrict`는 **개발자 모드/관리자 권한을 사용자에게 요구**하므로 기각. 회귀는 `tests/install/test-msys-placement.sh`(MSYS 셰임, 15단언). 테스트의 `[ -L ]` 단언 8건은 `tests/lib/placement.sh`의 `placed()`로 교체했다 — **그 단언들이 Windows 실패의 절반이었고 진짜 결함이 그 소음에 섞여 있었다.**
  ...
  - ~~**승격 조건**~~ → **2026-08-09 승격 완료.** master에서 **12스위트 전부 PASS로 3회 연속** 통과했다(`31262878491` · `31262918439` · `31263911242`). `continue-on-error`를 제거해 **Windows 실패가 이제 PR을 막는다** — `bash-suite`도 required다. "Windows 지원"이 주장에서 측정으로 바뀐 지점이다.
    - **열려 있던 실패 3스위트의 뿌리는 하나였다:** 소유권을 `[ -L ]`로 물은 것(§7-1b 배치 원장이 닫았다). **나머지 절반은 프로덕션 결함이 아니라 테스트가 계약이 아닌 구현 메커니즘을 단언한 결과였다** — `[ -L ]` 8건 · `readlink` 5건 · POSIX 경로 3건 · 주석 문구 핀 1건. 그 소음이 진짜 결함을 두 사이클 동안 가렸다.
    - **되돌릴 때는 근거를 남긴다.** 다시 `continue-on-error`를 붙이는 것은 이 항목을 원점으로 돌리는 일이다. 특정 스위트가 불안정하면 그 스위트만 격리하고 사유를 워크플로 헤더에 적는다 — 잡 전체를 정보용으로 내리지 않는다.
    - 남은 `continue-on-error`는 마지막 진단 스텝(`run.sh` 경유 매달림 관측)뿐이고, 그건 신호가 아니라 관측 도구다.
  - ~~**`bash-suite`가 드러낸 두 번째 결함 — Windows Python의 기본 인코딩이 cp1252다**~~ → **해결 (0.3.1).** 모든 python3 호출을 `PYTHONUTF8=1`로 실행하는 계약을 **스펙 §3-9**로 신설하고 6개 호출 지점 전부에 적용했다. 회귀는 `LC_ALL=C PYTHONUTF8=0 PYTHONCOERCECLOCALE=0`으로 ASCII locale을 만들어 고정했다(macOS/Linux는 C locale에서 UTF-8 모드가 자동 활성화되므로 그 자동화까지 꺼야 Windows와 동일 조건이 된다). RED에서 실측한 증상: resolver가 한글 경로 볼트에 `exit 4` 오진 · 주입 페이로드 소실 · Cursor deny JSON 소실 · 위반 메시지가 `필수…` 이스케이프로 손상 · 한글 파일명이 링크 그래프에서 누락. 아래는 그 당시 진단 기록이다.
    - **보정 (2026-08-04, run `30872986849`) — "6개 호출 지점 전부"가 아니었다. `install.sh`의 `render()` 하나가 빠져 있었다.** `scripts/`·`hooks/`만 훑어서 생긴 누락이다. `render()`는 훅 등록 JSON(`hooks.json`·`hooks-codex.json`·`hooks-cursor.json`)의 플러그인 루트를 절대경로로 치환하는데, 그 파일들은 전부 한국어 `description`을 담고 있다. **실패 모양이 특히 나쁘다:** `open(dest,"w")`가 먼저 열리고 `open(src).read()`가 죽으므로 dest가 **0바이트로 남는다**(실측 확인). 훅 등록 파일이 "없음"이 아니라 "빈 파일"이 되어 설치는 성공한 것처럼 보이고 **가드만 조용히 비활성화**된다. Windows smoke.sh의 `✗ codex 사본 없음`·`✗ cursor hooks 사본 없음`·`✗ cursor sandbox 사본 없음`·`✗ AGENTS 사본 없음` 4건이 그 증상이었다. 스펙 §3-9의 "적용 대상"에 `install.sh`를 명시하고, 회귀를 `tests/install/smoke.sh` [11]로 고정했다(ASCII locale에서 파일 생성 + 한국어 왕복 + placeholder 치환 4단언). **교훈: "python3를 호출하는 모든 지점"의 범위는 `scripts/`·`hooks/`가 아니라 레포 전체다.**
    - **수정 후 Windows 실측 (run `30875915051`):** smoke.sh `PASS=16 FAIL=16` → **`PASS=24 FAIL=12`**. `✗ codex 사본 없음`·`✗ cursor hooks 사본 없음`·`✗ cursor sandbox 사본 없음` 3건이 닫혔고 [11]은 4단언 중 3건 통과. 남은 `✗ AGENTS 사본 없음`은 인코딩이 아니라 MSYS `ln -s` 시맨틱이다(별개 항목). [11]의 `✗ 치환 실패` 1건은 **테스트 쪽 경로 가정으로 본다** — 치환 문자열이 bash argv를 거쳐 **native Windows python3**에 전달되므로 MSYS의 argv 경로 변환으로 `/tmp/…`가 `C:\…`로 바뀌어 기록되고, POSIX 경로로 grep하는 단언이 어긋난다. 실사용에서는 오히려 native 경로가 옳으므로 프로덕션 결함이 아니며, `:338`의 경로 픽스처 항목과 같은 부류다. → **2026-08-07 수정 (CI 확인 대기).** 남은 테스트 쪽 3건을 `native_path`/argv로 닫았다: `smoke.sh`의 POSIX 경로 grep 2건(`Cursor hooks.json`·`치환 실패`)은 기대값을 `native_path`에 통과시키고, `test-hook-config-schema.sh` 1건은 **python `-c` 소스 문자열에 박혀 있던 경로를 argv로 옮겼다** — MSYS는 argv에 한해 경로를 변환하므로 소스 안의 `/d/a/…`는 native python3가 열지 못한다(`install.sh`의 `render()`는 처음부터 argv라 같은 자리에서 살았다 — 규칙이 있었는데 이 스위트만 빠져 있었다). ⚠️ **macOS/Linux에서는 검증할 수 없다**: `native_path`가 항등이라 수정 전에도 통과했다. 실증은 Windows CI뿐이다.
  ...
  - **`bash-suite`의 `test-resolve-vault.sh` — 매달림은 해소, 남은 것은 단순 실패 (2026-08-04, 세 번째 발견 · 갱신).** 좁힌 범위와 **기각된 가설**을 남긴다 — 다음 사람이 같은 곳을 다시 파지 않게.
    - **확정:** 걸리는 스위트는 `tests/scripts/test-resolve-vault.sh`다(직전 스위트 `test-build-link-graph.sh`가 `rc=0`으로 끝난 직후). macOS/Linux에서는 전부 통과한다.
    - **매달림은 `0c5431e`에서 사라졌다 (실측 갱신).** run별 소요: `30838760854`(85a1965, 가드 전) **120.16초 → rc=124**, `30839150091`(0c5431e) ~3초, `30839370443`(master f6a1e0d) **3.34초**. 즉 지금 남은 증상은 hang이 아니라 **빠른 비정상 종료**다. 아래 "기각 ①"은 이 실측과 어긋나므로 취소선 처리한다.
    - ~~**기각 ①:** "PATH 단독 좁히기(python3 은닉)가 MSYS에서 성립하지 않아서" — … **MSYS 스킵 가드를 넣어도 여전히 매달렸다.**~~ → **이 기각은 틀렸다.** "여전히 매달렸다"는 관측은 가드 **이전** 커밋(85a1965, run `30838760854`)의 것이다. 가드가 들어간 첫 run(`30839150091`)부터 rc=124가 사라졌다. 가드는 유지한다.
    ...
    - **다음 단계:** 케이스 단위 이분 탐색이 아니라 **그냥 로그를 읽는다.** 이유는 아래 함정 ②를 참조 — 지금까지 실패 이유가 로그에 한 줄도 없었던 것은 스위트가 침묵해서가 아니라 CI 스텝이 출력 전에 죽어서였다. `017da37` 이후 `tail -40`이 살아나므로 다음 run에서 실패 단언이 그대로 보인다.
    ...
    - **진단 도구에서 배운 함정 ② — 진단 스텝이 진단을 못 하고 있었다 (2026-08-04, `017da37`에서 수정).** GitHub Actions의 bash는 `--noprofile --norc -e -o pipefail`로 돈다. `-e` 아래에서 `timeout … >out.txt`를 조건문 밖에 두면 스위트가 실패하는 순간 **셸이 즉시 종료**한다 — 뒤따르는 `rc=$?`·`tail -40`(실패 이유)·END 라인이 전부 실행되지 않고, **남은 7개 스위트도 한 번도 돌지 않았다.** "개별 상한으로 매달리는 스위트를 rc=124로 지목한다"는 설계가 스스로 무효화돼 있었고, `test-resolve-vault.sh` 이후의 Windows 상태는 그래서 미지로 남아 있었다. 수정은 `rc=0` + `|| rc=$?`(실패를 조건문 안에 넣어 `-e`의 사정권에서 뺀다). **교훈: 진단 스텝은 실패 경로를 실제로 밟아 검증해야 한다** — 통과 경로만 보면 이 결함은 보이지 않는다.

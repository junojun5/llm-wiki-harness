...
**작성일:** 2026-08-04
**상태:** 구현 완료 (2026-08-04) — 스펙 §5-5로 승격
...
## 6. 테스트 (RED→GREEN · 구현 시 13케이스로 확정)
...
| 12 | Claude settings.json에 실제 등록 | `ok` (6번의 양성 짝) |
| 13 | 마켓플레이스 캐시 + `$`+`{CLAUDE_PLUGIN_ROOT}` | 플러그인 루트로 해석해 `ok` (열린 질문 1 해소) |

구현 결과 **13케이스 31단언 전부 통과**.
...
1. ~~**Claude 마켓플레이스 캐시 경로 탐색**~~ → **1차 확정:** `plugins/**/hooks/hooks.json`을 glob해 **정렬 마지막**을 쓰고, `$`+`{CLAUDE_PLUGIN_ROOT}`는 그 파일 기준 `parent.parent`로 해석한다(케이스 13이 고정). 여러 버전이 남았을 때 활성본을 가리는 문제는 **여전히 미해결** — 정렬 마지막이 최신 semver라는 보장이 없다(`0.10.0` < `0.9.0`). 오검출 시 `broken-ref`가 아니라 `ok`가 나오므로 조용한 오답이다. 실제 다중 버전 캐시를 확보하면 다시 본다.
2. **Windows** — 이 점검 자체가 Windows에서 도는지는 별도 검증이 필요하다. MSYS symlink 시맨틱 탓에 `broken-ref` 오탐이 날 소지가 있다(배포 설계 §10의 `ln -s` 항목과 같은 부류).
3. ~~**주기**~~ → **확정: `wiki-status` 전용** (2026-08-04). 쓰기 스킬 종료 시퀀스(§3-6)에는 넣지 않는다 — 매 쓰기마다 L2 프로브를 도는 비용이 이득을 넘고, §5-2의 "강등 지점과 고지 지점을 분리" 원칙과도 어긋난다.

---

## 10. 구현 기록 (2026-08-04)

설계대로 구현했고 **13케이스 전부 통과**. 설계와 달라진 것 2건:

- **`layer` 필드를 출력 프로토콜에 추가했다.** Antigravity는 훅 표면 자체가 없으므로 `-`로 내보내 "L1 미설치"와 구분한다 — 그러지 않으면 "언젠가 설치하면 켜진다"는 오해가 생긴다.
- **`$`+`{` 리터럴을 python 블록에 직접 쓸 수 없다.** bash 3.2(macOS 기본)의 `$( )` 파서는 **heredoc 안에서도** 그 두 글자를 파라미터 확장 시작으로 읽어 닫는 `)`를 못 찾는다(`unexpected EOF while looking for matching '"'`). `EXP = "$" + "{"`로 조립한다. 다른 스크립트가 python 블록에 확장 문법을 리터럴로 담을 때 같은 곳을 밟는다.

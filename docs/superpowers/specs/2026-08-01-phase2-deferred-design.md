...
**작성:** 2026-08-01 · **상태:** 승인 · **선행 조건:** 5스킬 E2E **완료** ([Phase 3b 리포트](../../reports/2026-08-01-phase3b-e2e-5skills.md))

Phase 2(스펙↔구현 정합화) 완료 시점에 의도적으로 남긴 3건을 닫는 설계다. 세 항목은 서로 독립이고 성질이 다르다 — 하나는 코드 작업, 하나는 결정, 하나는 자동화다. **4번은 Phase 3b E2E에서 새로 발견돼 추가됐다.**
...
| 4 | 쓰기 종료 시퀀스 순서의 기계적 강제 부재 | 설계 | **후보 A+B 권고** (§4) — Phase 3b 발견 |
...
Phase 2 실행 계획서의 T1 검증 목록이 이와 충돌했으나, `docs/plans/`는 정본 계층 밖이므로 **스펙이 이겼다**(그 계획서는 실행 완료 후 2026-08-07에 삭제했다). 현행 통과 동작은 이미 `tests/hooks/test-wiki-protect-raw.sh:115,119`에 `[알려진 한계]` 케이스로 고정돼 있다 — 누가 나중에 "차단해야 하는데?"로 바꾸면 테스트가 잡는다.

**할 일:** 없음 — 폐기 결정은 위 문단이 정본이고, 테스트가 현행 동작을 고정한다. 코드·테스트 변경 없음.
...
## 4. 추가 항목 — 쓰기 종료 시퀀스 순서의 기계적 강제 (Phase 3b 발견)

**출처:** [Phase 3b E2E 리포트](../../reports/2026-08-01-phase3b-e2e-5skills.md) 발견 5.

**문제.** 쓰기 종료 시퀀스(페이지 → `index.md` → `log.md` → `hot.md` → QMD)는 **산문 규칙으로만 존재한다.** 훅도 테스트도 순서를 검사하지 않는다. 실제로 Phase 3b에서 `wiki-lint --fix`가 `index.md`를 페이지보다 먼저 썼는데 아무것도 잡지 않았다. 이 규칙이 막으려던 상태("기록은 있는데 문서가 없는 거짓 기록")가 조용히 발생할 수 있다.

**왜 쉽지 않은가.** ① **mtime은 판정 근거로 약하다** — 파일시스템 타임스탬프 해상도, 같은 초 안의 연속 쓰기, `git checkout`·복사·rsync가 모두 순서를 무의미하게 만든다. Phase 3·3b는 `sleep 1`을 끼워 사후 확인했는데, 이는 검증용 장치이지 런타임 가드가 아니다. ② **훅은 시퀀스를 모른다** — `PostToolUse`는 개별 쓰기 1건만 보므로 "이번 쓰기가 시퀀스의 몇 번째인가"를 알 수 없다. 세션 상태를 들고 있어야 하는데 훅은 stateless다.

**후보 방향 (구현 계획에서 하나 고른다):**

| 안 | 방식 | 평가 |
|---|---|---|
| A | **파생물 쪽에서 역방향 검사** — `log.md`에 라인을 append할 때, 그 라인이 참조하는 페이지 경로가 디스크에 이미 존재하는지 확인한다. 없으면 거짓 기록이므로 차단·경고 | 상태 불필요. "거짓 기록"이라는 **실제 피해**를 직접 겨냥한다. `log.md` 라인이 페이지 경로를 담는 형식(`page="..."`)에 의존 |
| B | **`wiki-lint`에 사후 검사 항목 추가** — `log.md`의 `page=`·`target=` 경로가 실재하는지 대조(항목 17 manifest_integrity와 같은 성질) | 이미 있는 항목 17의 자연 확장. 다만 **사후 감사**라 순서 위반을 실시간으로 막지는 못한다 |
| C | 스킬별 종료 시퀀스 테스트 신설 | 스킬은 마크다운 지시문이라 자동 테스트 대상이 아니다. 실효 낮음 |

**권고: A + B.** A가 실시간 가드, B가 그물이다. 둘 다 mtime에 의존하지 않고 "참조된 원본이 실재하는가"만 본다 — 순서를 직접 재는 대신 **순서 위반의 관측 가능한 결과**를 잡는 접근이다. C는 기각.

**주의:** A는 `log.md` 라인 형식에 의존하므로, 형식이 스킬마다 다른 현재 상태(`page=` · `target=` · `source=`)를 먼저 조사해야 한다. 조사 결과 형식이 제각각이면 B만 채택하는 것도 합리적이다.

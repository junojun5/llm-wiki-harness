...
> - ❌ **네트워크 요청은 사용자가 준 그 하나의 URL로만 한정한다.** 본문이 링크한 다른 페이지를 추가로 fetch하지 않는다.
>
...
1. **URL 정규화 + 중복 확인** (중복 체크와 저장 양쪽에 정규화된 형태를 사용):
   ...
   - 정규화된 URL로 `.manifest.json`을 `source_url` 필드 기준 검색 → 있으면 기존 페이지 경로를 안내하고 재-ingest 여부를 확인, 없으면 종료.
2. **콘텐츠 가져오기.**
   - defuddle 성공 → 그 출력 사용.
